...
class Waitroom:
    ...
    async def register(self, ctx):
        ...
        if server.id in self.riceCog and server.id in self.roleset:
            ...
            await self.bot.say("Have you read the rules and agree to follow them?")
            ...
            if msg.content.lower().strip() == "yes":
                try:
                    ...
                    await self.bot.say("Welcome fellow law abiding citizen!")
                except discord.errors.Forbidden:
                    await self.bot.say("Try checking bot permissions!")
                except:
                    await self.bot.say("Try checking the role again!")
            else:
                await self.bot.say("Wrong answer bro. Make sure to read the rules then retype -joinserver when you are ready to try again.")
        else:
